...
%Step1 = find(p.FC0_Set == 0.8900);
%Step2 = find(p.FC0_Set == 1.9389);
%Step3 = find(p.FC0_Set == 4.03);

% 3600 s chosen since chamber equilibrates within this time
%p.hcho([Step1(1:3600); Step2(1:3600); Step3(1:3600)]) = NaN;


 Step1_bypass = find(p.FC0_Set == 0.8900 & p.Flag == 51);
% Step2_bypass = find(p.FC0_Set == 1.9389 & p.Flag == 51);
% Step3_bypass = find(p.FC0_Set == 4.03 & p.Flag == 51);
% 
 Step1_chamber = find(p.FC0_Set == 0.8900 & p.Flag == 50);
% Step2_chamber = find(p.FC0_Set == 1.9389 & p.Flag == 50);
% Step3_chamber = find(p.FC0_Set == 4.03 & p.Flag == 50);
% 
